...
let () =
  ...
  run_test "log_order" (fun () ->
      ...
      let msgs =
        List.map
          (fun n -> (Fit.Commit.metadata (Fit.Node.commit n)).message)
          log
      in
  ...
  run_test "checkout_root" (fun () ->
      ...
      assert_true "file deleted after checkout to root"
        (not (file_exists "f.txt"));
      let d = assert_ok "diff" (Fit.diff repo) in
  ...
  run_test "feature_isolation" (fun () ->
      ...
      assert_true "feat more commits"
        (List.length feat_log > List.length master_log));
