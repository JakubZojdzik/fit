...
type t = { meta : metadata; diff : Diff.t }
...
let generate_id diff message author timestamp =
  let content =
    Printf.sprintf "%s|%s|%f|%s" message author timestamp (Diff.to_string diff)
  in
  hash_string content
...
let initial message author = create Diff.empty message author
...
let apply commit state =
  List.fold_left
    (fun current_state file_diff ->
      let path = file_diff.Diff.path in
      let old_content =
        match List.assoc_opt path current_state with Some c -> c | None -> []
      in
...
let to_string commit =
  Printf.sprintf
    "COMMIT\n\
     ...
     %s\n\
     END_COMMIT"
    commit.meta.id commit.meta.message commit.meta.author commit.meta.timestamp
