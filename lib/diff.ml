...
let compute_file_diff path (old_content : File.text_content) (new_content : File.text_content) =
  let old_file = write_temp "fit_old_" old_content in
  let new_file = write_temp "fit_new_" new_content in
  let cmd = Printf.sprintf "diff -u %s %s 2>/dev/null || true"
    (Filename.quote old_file) (Filename.quote new_file) in
...
let compute_state_diff (old_state : File.state) (new_state : File.state) =
  let old_map = List.fold_left (fun acc (path, content) ->
    StringMap.add path content acc
  ) StringMap.empty old_state in
  let new_map = List.fold_left (fun acc (path, content) ->
    StringMap.add path content acc
  ) StringMap.empty new_state in

  (* modified or removed files *)
  let from_old =
    StringMap.fold (fun path old_content acc ->
      let new_content =
        match StringMap.find_opt path new_map with
        | Some c -> c
        | None -> []
      in
  ...
  (* new files *)
  let from_new =
    StringMap.fold (fun path new_content acc ->
...
let apply_file_diff (reverse : bool) (file_diff : file_diff) (content : File.text_content) : File.text_content =
  if file_diff.patch = "" then content
    ...
    let cmd = Printf.sprintf "patch %s-u -s %s %s 2>/dev/null"
      reverse_flag (Filename.quote input_file) (Filename.quote patch_file) in
    let ret = Sys.command cmd in
    let result =
      if ret = 0 then read_file_lines input_file
      else content
    in
