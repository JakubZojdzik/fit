...
let find_node_by_id history node_id =
  ...
  and dfs_children children visited =
    ...
    | [] -> None
    | child :: rest -> begin
        match dfs visited child with
        | Some _ as found -> found
        | None -> dfs_children rest visited
      end
    end
  in
  match history.root with None -> None | Some root -> dfs StringSet.empty root
...
let checkout_branch history branch_name =
  ...
  | Some node ->
      Result.return
        { history with head = Some node; current_branch = Some branch_name }
...
let revert_to history node_id =
  ...
  | Some branch_name -> (
      ...
      | Some node ->
          ...
          Result.return
            { history with head = Some node; branches = new_branches })
...
let list_branches history = BranchMap.bindings history.branches |> List.map fst
    ...
    | Some secondary_node -> begin
        ...
        | Some primary_node ->
            ...
            let merged_state =
              ...
              let secondary_only =
                List.filter
                  (fun (p, _) -> not (List.mem p primary_paths))
                  secondary_state
              in
              primary_state @ secondary_only
            in
            let merged_diff =
              Diff.compute_state_diff primary_state merged_state
            in
            let merge_commit = Commit.create merged_diff message author in
            let merge_node =
              Node.create_merge merge_commit primary_node secondary_node
            in
            Result.return
              {
                history with
...
let reconstruct_head_state history =
  match history.head with None -> [] | Some node -> reconstruct_state node
