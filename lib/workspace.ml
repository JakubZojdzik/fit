  ...
  if Sys.file_exists path then
    ...
    let rec read acc =
      ...
      | line ->
        ...
        if trimmed = "" then read acc
        else read (trimmed :: acc)
...
let rec collect_files ignore_list base_path rel_path =
  ...
  if should_ignore ignore_list (Filename.basename full_path) then []
    ...
    List.concat_map (fun entry ->
      let new_rel = if rel_path = "." then entry else Filename.concat rel_path entry in
  ...
  try
    ...
    let rec remove_empty_parents dir =
      if dir <> workspace_path 
         ...
         && Array.length (Sys.readdir dir) = 0 then begin
        Unix.rmdir dir;
...
let read_state workspace_path =
  ...
  let results = List.filter_map (fun rel_path ->
...
let write_state workspace_path state =
  ...
  let to_delete = List.filter (fun f -> not (List.mem f target_paths)) current_files in
  let* () = List.fold_left (fun acc rel_path ->
    let* () = acc in
    delete_file workspace_path rel_path
  ) (Result.return ()) to_delete in
  List.fold_left (fun acc (rel_path, content) ->
    let* () = acc in
    write_file workspace_path rel_path content
  ) (Result.return ()) state

let apply_diff reverse workspace_path diff =
  List.fold_left (fun acc (file_diff : Diff.file_diff) ->
    let* () = acc in
    let current_content = 
      ...
      | Result.Error _ -> []
    in
    let new_content = Diff.apply_file_diff reverse file_diff current_content in
    if new_content = [] then
      delete_file workspace_path file_diff.path
    else
      write_file workspace_path file_diff.path new_content
  ) (Result.return ()) diff
